...
def pointsFromSTL( address, sx, size, center, alpha ) :
    # Extract the mesh and the points

    shipMesh = mesh.Mesh.from_file(address)
...
def polygonFromSTL(address,sx,scale,center,alpha,minlen,maxlen):
    points  = pointsFromSTL(address,sx,scale,center,alpha)
    xlist  = points[0]
    ylist  = points[1]
    factor = points[2]
...
def polygonFromBlend(address,sx,scale,center,alpha,minlen,maxlen):
    ...
    stlAdress = (os.path.basename(address) + ".stl")
...
if __name__ == "__main__":
    ...
    polygonify_all_ships_stl("../ships/", "./ship_polygon_3d/", 0)
